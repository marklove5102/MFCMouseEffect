import {
  ...
  UPSERT_GROUP_PRESENTATION_COMMAND_BYTES,
...
const GROUP_ID: u32 = 6210;
const GLOW_ID: u32 = 6211;
const TRAIL_ID: u32 = 6212;
const FIELD_ID: u32 = 6213;
const TRAIL_POINT_COUNT: u32 = 5;
const FIELD_ITEM_COUNT: u32 = 5;
const PRESENT_BYTES: u32 = UPSERT_GROUP_PRESENTATION_COMMAND_BYTES;
...
const LEFT_TOTAL_BYTES: u32 = PRESENT_BYTES + GLOW_BYTES + TRAIL_BYTES + FIELD_BYTES;
...
export function mfx_plugin_on_input(
  ...
  if (button == BUTTON_MIDDLE) {
    if (outputCap < PRESENT_BYTES) {
      return 0;
    }
    writeUpsertGroupPresentation(outputPtr, <f32>0.24, GROUP_ID, true);
  ...
  writeUpsertGroupPresentation(outputPtr, <f32>1.0, GROUP_ID, true);
  emitGlow(outputPtr + <usize>PRESENT_BYTES, x, y);
  emitRibbon(outputPtr + <usize>(PRESENT_BYTES + GLOW_BYTES), x, y);
  emitQuadField(outputPtr + <usize>(PRESENT_BYTES + GLOW_BYTES + TRAIL_BYTES), x, y, seed);
