import {
  ...
  UPSERT_GROUP_CLIP_RECT_COMMAND_BYTES,
...
const GROUP_ID: u32 = 7410;
const GLOW_ID: u32 = 7411;
const TRAIL_ID: u32 = 7412;
const FIELD_ID: u32 = 7413;
const TRAIL_POINT_COUNT: u32 = 5;
const FIELD_ITEM_COUNT: u32 = 5;

const CLIP_BYTES: u32 = UPSERT_GROUP_CLIP_RECT_COMMAND_BYTES;
...
const LEFT_TOTAL_BYTES: u32 = CLIP_BYTES + GLOW_BYTES + TRAIL_BYTES + FIELD_BYTES;

function emitGroupClip(outputPtr: usize, x: f32, y: f32, widthPx: f32, heightPx: f32): void {
  writeUpsertGroupClipRect(
    outputPtr,
    x - widthPx * <f32>0.5,
    y - heightPx * <f32>0.5,
...
export function mfx_plugin_on_input(
  ...
  if (button == BUTTON_MIDDLE) {
    if (outputCap < CLIP_BYTES) {
      return 0;
    }
    emitGroupClip(outputPtr, x + <f32>52.0, y, <f32>112.0, <f32>92.0);
    return CLIP_BYTES;
  }
  ...
  emitGroupClip(outputPtr, x, y, <f32>180.0, <f32>132.0);
  emitGlow(outputPtr + <usize>CLIP_BYTES, x, y);
  emitRibbon(outputPtr + <usize>(CLIP_BYTES + GLOW_BYTES), x, y);
  emitQuadField(outputPtr + <usize>(CLIP_BYTES + GLOW_BYTES + TRAIL_BYTES), x, y, seed);
