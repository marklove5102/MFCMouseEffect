...
private func mfxPathResolveLineJoin(_ value: UInt8) -> CAShapeLayerLineJoin {
    switch value {
    case 0:
        return .miter
    case 2:
...
private func mfxPathResolveLineCap(_ value: UInt8) -> CAShapeLayerLineCap {
    switch value {
    case 0:
        return .butt
    case 2:
...
@MainActor
private func mfxCreateWasmPathStrokeOverlayOnMainThread(
    frameX: Double,
    frameY: Double,
    frameSize: Double,
    ...
    let resolvedLineWidth = mfxPathClamp(CGFloat(lineWidthPx), min: 0.5, max: 48.0)
    let resolvedAlpha = mfxPathClamp(CGFloat(alphaScale), min: 0.0, max: 1.0)
    let resolvedLineJoin = mfxPathResolveLineJoin(lineJoin)
    let resolvedLineCap = mfxPathResolveLineCap(lineCap)
    ...
    for glowPass in 0..<2 {
        ...
        glow.strokeColor = mfxPathColorFromArgb(
            glowArgb,
            alphaScale: resolvedAlpha * (glowPass == 0 ? 0.38 : 0.22)
        ).cgColor
        glow.lineWidth = resolvedLineWidth + CGFloat(10 + glowPass * 5)
        glow.lineJoin = resolvedLineJoin
        glow.lineCap = resolvedLineCap
    ...
    stroke.strokeColor = mfxPathColorFromArgb(strokeArgb, alphaScale: resolvedAlpha).cgColor
    stroke.lineWidth = resolvedLineWidth
    stroke.lineJoin = resolvedLineJoin
    ...
    content.layer?.add(group, forKey: "mfx_wasm_path_stroke_overlay")

    return Unmanaged.passRetained(window).toOpaque()
}

@MainActor
@_cdecl("mfx_macos_wasm_path_stroke_overlay_create_v1")
public func mfx_macos_wasm_path_stroke_overlay_create_v1(
    _ frameX: Double,
    _ frameY: Double,
    _ frameSize: Double,
    _ nodesPtr: UnsafeRawPointer?,
    ...
    return mfxCreateWasmPathStrokeOverlayOnMainThread(
        frameX: frameX,
        frameY: frameY,
        frameSize: frameSize,
        nodes: nodes,
...
@MainActor
@_cdecl("mfx_macos_wasm_path_stroke_overlay_show_v1")
public func mfx_macos_wasm_path_stroke_overlay_show_v1(_ windowHandle: UnsafeMutableRawPointer?) {
