...
void BuildWin32MouseCompanionRealRendererAppendages(
    ...
    scene.tailRect = Gdiplus::RectF(
        scene.bodyRect.GetRight() - metrics.bodyWidth * style.tailXOffsetRatio -
            ...
            profile.idleTailSway * scene.facingSign,
    ...
    scene.leftEar = {{
        ...
        Gdiplus::PointF(
            scene.centerX - scene.headRect.Width * style.earTipSpreadRatio + poseEarSpread - profile.earSwing * style.leftEarSwingScale -
                profile.idleHeadSway,
            earTipY),
        Gdiplus::PointF(
    ...
    scene.rightEar = {{
        ...
        Gdiplus::PointF(
            scene.centerX + earBaseOffset + scene.headRect.Width * style.earOuterSpreadRatio + poseRightEarSpread -
                profile.earSwing + profile.idleHeadSway,
            scene.headRect.Y + scene.headRect.Height * style.earBaseYRatio),
        Gdiplus::PointF(
            scene.centerX + scene.headRect.Width * style.earTipSpreadRatio + poseRightEarSpread - profile.earSwing * style.rightEarSwingScale +
                profile.idleHeadSway,
            rightEarTipY),
        Gdiplus::PointF(
            scene.centerX + scene.headRect.Width * style.earRootInsetRatio,
    ...
    scene.leftHandRect = Gdiplus::RectF(
        scene.bodyRect.X - metrics.bodyWidth * style.handXOffsetRatio,
