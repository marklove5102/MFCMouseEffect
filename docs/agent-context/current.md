...
- `assetNodeTargetResolverProfile` now feeds `assetNodeMatchCandidateProfile -> assetNodeMatchCatalogProfile -> assetNodeMatchEnumerationProfile -> assetNodeMatchPlanProfile -> assetNodeMatchResolveProfile`: candidate entries derive selector prefixes and candidate paths, catalog normalizes canonical keys/labels plus `matchConfidence`, enumeration emits parser-style locators, plan assembles probe/fallback intent, and the new resolve layer now exposes final node keys/labels plus `resolveConfidence` consumed by `assetNodeWorldSpaceProfile`, `assetNodeJointHintProfile`, `FrameBuilder`, `AdornmentBuilder`, and `ActionOverlayBuilder`.
