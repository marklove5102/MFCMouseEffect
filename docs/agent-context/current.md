...
- Effects:
  ...
  - 2026-03-03 macOS overlay refresh sync: new config/state/schema field `overlay_target_fps` is wired (`0=auto follow display max refresh, >0=explicit cap`), and Swift overlay support now exposes dynamic timer interval resolution (`mfx_macos_overlay_timer_interval_ms_v1`) consumed by line-trail rendering to avoid fixed low-refresh cadence.
  - Trail `none` hard-disable, line-trail diagnostics, and anti-origin-connector guards are in place.
  - 2026-03-03 trail parity hardening (macOS): `line/streamer/electric/meteor/tubes` now use one continuous line-trail path (particle keeps segment pulse path), eliminating pulse-style "matchstick" fragmentation for non-particle trail types.
