...
- POSIX dual-lane guardrail exists:
  ...
  - `PosixScaffoldAppShell` is now split by concern (`Actions`/`Stdin` + lifecycle entry), and core/scaffold stdin-exit parsing is consolidated via shared `PosixShellExitCommand`, reducing dual-lane shell drift risk while preserving launch/exit contracts
