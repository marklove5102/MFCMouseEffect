...
- The settings shell now uses an integrated, low-noise `header -> status -> tabs -> content` frame: tabs no longer carry automation diagnostics, the old current-section label/title block has been removed, the active-section description remains as a compact single-line hint, the automation debug card now renders inside the main content column, the header subtitle is compressed to a single metadata line, `Apply` remains the only high-weight solid button, the other header actions are now icon-first with a shared inline-SVG icon set and lighter/ghosted styling, the status bar now stays thinner in the default short-text state while still expanding for long diagnostics, and section sub-tabs should remain visibly lighter than the top-level navigation.
