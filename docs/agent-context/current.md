...
- Runtime diagnostics now expose both lane and scene-runtime state directly: `default_lane_candidate / source / rollout_status / style_intent / candidate_tier`, `appearance_plugin_sample_tier`, `appearance_plugin_contract_brief`, plus `scene_runtime_adapter_mode / pose_sample_count / bound_pose_sample_count`; adapter modes are fixed to `runtime_only | pose_unbound | pose_bound`.
- Current `pose_bound` gain is intentionally narrow but visible: appendage pose consumption now scales by adapter mode, and it also lightly biases `body_forward`, `head_nod`, and face focus (`pupil/brow`) from pose samples.
- `pose_bound` now also reaches frame/face anchors directly: head/body center offsets, eye/nose/mouth placement, blush lift, and whisker spread/tilt all receive a bounded pose bias, so Windows real preview is no longer only pose-aware through appendage + motion lanes.
- `pose_bound` now reaches accessory/overlay/grounding layers too: pose badge visibility prefers adapter-aware state over raw booleans, accessory anchors track the same bounded pose offset, click/hold/scroll/drag/follow overlays inherit a small pose-led center shift, and shadow/pedestal grounding now drifts/scales slightly with the bound leg lane instead of staying purely action-driven.
