...
- `assetNodeTargetResolverProfile` now feeds a new `assetNodeMatchCatalogProfile` with structured placeholder match results (`resolvedNodeKey`, `resolvedNodeLabel`, `canonicalNodeKey`, `canonicalNodeLabel`, `matchConfidence`), and downstream `assetNodeWorldSpaceProfile` / `assetNodeJointHintProfile` now consume that catalog instead of raw path-only resolution.
