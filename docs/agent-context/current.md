...
- `SceneRuntime` now also owns `assetNodeTargetProfile` and `assetNodeTargetResolverProfile`, lifting parent-space values into shared per-node target entries and then into asset-path-aware resolved target entries before anchor generation; the real renderer backend then derives post-scene `assetNodeWorldSpaceProfile`, `assetNodePoseProfile`, `assetNodePoseResolverProfile`, `assetNodePoseRegistryProfile`, `assetNodePoseChannelProfile`, and `assetNodePoseConstraintProfile`, so builders consume target-resolver seams, painter readability consumes world-space/pose/pose-registry/channel/constraint seams, and runtime/proof/matrix/WebUI surface `scene_runtime_asset_node_target_*`, `scene_runtime_asset_node_target_resolver_*`, `scene_runtime_asset_node_world_space_*`, `scene_runtime_asset_node_pose_*`, `scene_runtime_asset_node_pose_resolver_*`, `scene_runtime_asset_node_pose_registry_*`, `scene_runtime_asset_node_pose_channel_*`, and `scene_runtime_asset_node_pose_constraint_*`.
