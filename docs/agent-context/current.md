...
- `assetNodeTargetResolverProfile` now feeds `assetNodeMatchCandidateProfile -> assetNodeMatchCatalogProfile -> assetNodeMatchEnumerationProfile`: candidate entries derive selector prefixes, primary/secondary node names, and candidate paths from asset resources, the catalog turns them into canonical node keys/labels plus `matchConfidence`, and the new enumeration layer emits parser-style locators/labels that `assetNodeWorldSpaceProfile`, `assetNodeJointHintProfile`, `FrameBuilder`, `AdornmentBuilder`, and `ActionOverlayBuilder` now consume.
