...
- `SceneRuntime` now also owns `modelNodeAdapterProfile`, so frame/face/adornment/overlay/grounding consume one cached node-offset seam; runtime/proof/matrix/WebUI surface `scene_runtime_model_node_adapter_influence` and `scene_runtime_model_node_adapter_brief`.
