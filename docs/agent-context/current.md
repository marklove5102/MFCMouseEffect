...
- `assetNodeTargetResolverProfile` now feeds `assetNodeMatchCandidateProfile -> assetNodeMatchCatalogProfile -> assetNodeMatchEnumerationProfile -> assetNodeMatchPlanProfile`: candidate entries derive selector prefixes and candidate paths, catalog normalizes canonical keys/labels plus `matchConfidence`, enumeration emits parser-style locators, and the new plan layer now produces probe labels/confidence consumed by `assetNodeWorldSpaceProfile`, `assetNodeJointHintProfile`, `FrameBuilder`, `AdornmentBuilder`, and `ActionOverlayBuilder`.
