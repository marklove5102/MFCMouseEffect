...
- `WebUIWorkspace` now has a unified source-mode dev entry through `mfx`: `./mfx fast --debug --web` (or `./mfx start --debug --web` when a fresh native rebuild is required) starts the macOS host first, then launches/reuses the Vite dev server, opens the Vite URL with the current token, and keeps `/api/*` proxied back to the live host via the existing probe/runtime discovery chain. Raw `pnpm --dir MFCMouseEffect/WebUIWorkspace run dev` still works as a lower-level fallback, but the preferred frontend-debug entrypoint is now the `--web` flag on `mfx`.
- Static WebUI parity contract: `pnpm --dir MFCMouseEffect/WebUIWorkspace run build` must copy Vite-extracted per-entry Svelte CSS (`*.svelte.css`) into `MFCMouseEffect/WebUI`, and `MFCMouseEffect/WebUI/index.html` must load those CSS files after `styles.css`; otherwise `./mfx start --debug` can show stale/pre-Svelte styling while `./mfx start --debug --web` shows the current Vite dev styling.
...
- macOS daily shortcut: `./mfx run-no-build` / `./mfx fast` skip both core and WebUI rebuilds; `./mfx run` / `./mfx start` perform fresh build preparation; appending `--web` switches the browser target from the built-in static WebUI to the Vite source-mode dev UI without changing the host build policy.
