...
- POSIX dual-lane guardrail exists:
  ...
  - macOS tray menu factory now splits action-bridge and menu-item helpers into dedicated modules (`MacosTrayMenuFactory.ActionBridge.mm`, `MacosTrayMenuFactory.Items.mm`), reducing tray assembly coupling while preserving settings/exit menu behavior contracts
  ...
  - macOS hold style now splits accent geometry/render helpers into dedicated module (`MacosHoldPulseOverlayStyle.Accent.mm`), reducing type/color-vs-accent coupling while preserving hold visual contracts
