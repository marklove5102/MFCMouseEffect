...
- Phase0 host scaffold is active:
  ...
  - Windows Phase1.5 closure contract is now explicit: the current placeholder lane is expected to exit as a stable visible backend instead of growing indefinitely. The concrete exit gate now lives in `/Users/sunqin/study/language/cpp/code/MFCMouseEffect/docs/architecture/windows-mouse-companion-phase15-exit-contract.md`.
  - Windows real-renderer preparation contract is now explicit too: future richer rendering work must plug in behind the existing host/presenter/runtime lanes and keep `model / action_library / appearance_profile / PetPoseFrame / runtime action` as the stable upstream inputs. The renderer-facing boundary now lives in `/Users/sunqin/study/language/cpp/code/MFCMouseEffect/docs/architecture/windows-mouse-companion-real-renderer-contract.md`.
  ...
  - Windows placeholder facing pass is now active: horizontal follow/drag motion updates a simple left/right facing hint, and a small accumulated-motion hysteresis now suppresses rapid left/right flip jitter from tiny pointer shakes.
  - Windows placeholder motion helper is now active: idle/follow procedural bob, ear sway, paw cadence, tail swing, blink, and shadow squash are computed before scene assembly so the 2D pet feels less static without coupling more logic into the renderer.
  - Windows placeholder reactive-motion pass is now active: `click_react / hold_react / scroll_react / drag` feed a separate time-decayed reaction helper before scene assembly, so the pet now has impact/rebound, press compression, and directional scroll kick instead of only switching static silhouettes.
  - Windows placeholder face/detail pass is now active: whiskers, blush, mouth openness, chest bob, and accent glow all reuse the same reactive timeline, so action reactions are no longer limited to body silhouette changes.
  - Windows renderer-backend adapter groundwork is now active too: `Win32MouseCompanionWindow` no longer hardcodes the placeholder renderer type directly and now resolves a default renderer backend through a small factory/interface seam. The current placeholder path is still the only backend, but the host/window stack can now accept a future richer backend without reopening layered-window plumbing.
  - Windows renderer input seam is now active as well: the backend path no longer consumes the full `Win32MouseCompanionVisualState` directly. `Win32MouseCompanionRendererInput` + its builder now isolate renderer-facing action/pose/appearance/runtime fields from host/window-only fields such as pointer-anchor and visibility lifecycle state.
  - Windows placeholder painter split is now active too: the placeholder backend no longer owns both runtime interpretation and GDI+ paint code in one file. `Win32MouseCompanionPlaceholderRenderer` now coordinates `input -> runtime -> scene`, `Win32MouseCompanionPlaceholderSceneBuilder` owns placeholder scene assembly, and `Win32MouseCompanionPlaceholderPainter` owns the actual draw path. This keeps later renderer/backend swaps from being tied to one large paint-and-state file.
- Windows renderer runtime adapter is now active too: `Win32MouseCompanionRendererRuntime` centralizes action-state decoding, pose-sample lookup, normalized intensities, facing sign, and clip access before placeholder motion/posture/action-profile/scene helpers run. This keeps later renderer backends from having to re-interpret raw host input in several places.
  - Windows placeholder expression helper is now active: pupils, brows, and mouth-arc semantics are assembled behind a dedicated expression layer instead of being hardcoded in the renderer, keeping face behavior reusable for later presenter growth.
  - Windows placeholder adornment helper is now active: collar/charm placement and lightweight motion dust are now assembled separately from scene layout and renderer plumbing, so decorative/motion accents can grow without bloating the core pet geometry path.
  - Windows placeholder gait/accessory helpers are now active: neck/head-body bridge, limb bridge lines, and simple appearance-driven head accessories are assembled behind dedicated helpers, so the 2D placeholder no longer relies only on isolated body-part ellipses for structure.
  - Windows placeholder palette helper is now active: body/head/ear/accent materials plus tail-tip and paw-pad colors are resolved behind a dedicated palette layer, so appearance variant + tint changes no longer stay embedded inside scene layout code.
  - Windows placeholder posture helper is now active: `idle / follow / drag / hold / scroll` now establish different base body/head anchors, ear spread, stance width, and shadow/body bias before motion, rhythm, and pose offsets are layered on top.
  - Windows placeholder action-profile helper is now active: `click / hold / scroll / drag / follow` now also reshape chest lift, tail root/curl, shoulder/hip patch scale, ear-root read, and front/rear depth bias so action-specific silhouettes differ even before micro-motion is noticed.
  - Windows placeholder rhythm helper is now active: ear/tail/fore-leg/rear-leg phase relationships are now assembled behind a dedicated rhythm layer, so stride-like silhouette timing is no longer hardcoded only inside scene geometry formulas.
  - Windows placeholder silhouette helper is now active: ear roots, tail root, shoulder/hip patches, and front/rear depth nodes are assembled behind a dedicated shape layer so readability tweaks no longer stay embedded in renderer or scene plumbing.
