...
- Runtime diagnostics now expose both lane and scene-runtime state directly: `default_lane_candidate / source / rollout_status / style_intent / candidate_tier`, `appearance_plugin_sample_tier`, `appearance_plugin_contract_brief`, plus `scene_runtime_adapter_mode / pose_sample_count / bound_pose_sample_count / pose_adapter_influence / pose_readability_bias / pose_adapter_brief`; adapter modes are fixed to `runtime_only | pose_unbound | pose_bound`.
...
- `default_lane_source` stable machine values: `runtime_builtin_default`, `env_builtin_forced`, `env_wasm_candidate`, `env_wasm_fallback_builtin`, `runtime_plugin_candidate`.
