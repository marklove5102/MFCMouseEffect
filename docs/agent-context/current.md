...
- `SceneRuntime` now also owns `assetNodeTargetProfile`, lifting parent-space values into shared per-node target entries before anchor generation; builders now consume target seams instead of raw parent-space numbers, and runtime/proof/matrix/WebUI surface `scene_runtime_asset_node_target_*`.
