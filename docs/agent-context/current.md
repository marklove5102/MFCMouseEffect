...
- `WebUIWorkspace` source-mode dev is now owned by `--debug`: `./mfx fast --debug` (or `./mfx start --debug` when a fresh native rebuild is required) starts the macOS host first, launches/reuses the Vite dev server, opens the Vite URL with the current token, and proxies `/api/*` back to the live host. `./mfx start` remains the release-near static validation path.
- `Automation Mapping` must not call Svelte 5 exported component methods through `instance[name]()` in Vite dev mode. The editor now exposes plain `read/validate` closures through `onReady(api)` so `Apply`, `Reload`, and row mutations keep working under HMR without triggering `Cannot read from private field`.
