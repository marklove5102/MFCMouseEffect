...
- `SceneRuntime` now also owns `assetNodeTargetProfile` and `assetNodeTargetResolverProfile`, lifting parent-space values into shared per-node target entries and then into asset-path-aware resolved target entries before anchor generation; the real renderer backend then derives post-scene `assetNodeWorldSpaceProfile`, `assetNodePoseProfile`, `assetNodePoseResolverProfile`, `assetNodePoseRegistryProfile`, `assetNodePoseChannelProfile`, `assetNodePoseConstraintProfile`, `assetNodePoseSolveProfile`, `assetNodeJointHintProfile`, `assetNodeArticulationProfile`, `assetNodeLocalJointRegistryProfile`, `assetNodeArticulationMapProfile`, `assetNodeControlRigHintProfile`, `assetNodeRigChannelProfile`, and `assetNodeControlSurfaceProfile`, so builders consume target-resolver seams, painter readability consumes world-space/pose/registry/channel/constraint/solve/joint-hint/articulation/local-joint/articulation-map/control-rig/rig-channel/control-surface seams, and runtime/proof/matrix/WebUI surface `scene_runtime_asset_node_target_*`, `scene_runtime_asset_node_target_resolver_*`, `scene_runtime_asset_node_world_space_*`, `scene_runtime_asset_node_pose_*`, `scene_runtime_asset_node_pose_resolver_*`, `scene_runtime_asset_node_pose_registry_*`, `scene_runtime_asset_node_pose_channel_*`, `scene_runtime_asset_node_pose_constraint_*`, `scene_runtime_asset_node_pose_solve_*`, `scene_runtime_asset_node_joint_hint_*`, `scene_runtime_asset_node_articulation_*`, `scene_runtime_asset_node_local_joint_registry_*`, `scene_runtime_asset_node_articulation_map_*`, `scene_runtime_asset_node_control_rig_hint_*`, `scene_runtime_asset_node_rig_channel_*`, and `scene_runtime_asset_node_control_surface_*`.
