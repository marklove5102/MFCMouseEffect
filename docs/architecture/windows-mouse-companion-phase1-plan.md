...
- Backend-selection observability is now active too:
  ...
  - a native Windows `.cmd -> .ps1` proof helper now exists too: `tools/platform/manual/run-windows-mouse-companion-render-proof.cmd`, so the shortest bring-up path can run directly from Command Prompt / PowerShell without requiring Git Bash
