...
- renderer lane matrix expectations:
  ...
  - runtime / proof / lane matrix now also expose `scene_runtime_model_asset_source_brief`, `scene_runtime_model_asset_manifest_brief`, `scene_runtime_model_asset_catalog_brief`, `scene_runtime_model_asset_binding_table_brief`, `scene_runtime_model_asset_registry_brief`, `scene_runtime_model_asset_load_brief`, `scene_runtime_model_asset_decode_brief`, `scene_runtime_model_asset_residency_brief`, `scene_runtime_model_asset_instance_brief`, and `scene_runtime_model_asset_activation_brief` before the older `model_scene_adapter` / `model_node_adapter` chain, so you can tell whether the current Windows real renderer is still preview-only, has a supported model asset source, has already resolved the minimal `model + action library + appearance profile` manifest, has already lifted that manifest into registry/load/decode/residency, and has already crossed into the first runtime instance/activation seams needed for later true 3D consumption
